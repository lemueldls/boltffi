...
export CARGO_TARGET_DIR="$ROOT_DIR/benchmarks/rust-boltffi/target"
(
    cd "$DEMO_DIR"
...
cp -R "$ROOT_DIR/benchmarks/rust-boltffi/dist/wasm/pkg/." "$GENERATED_DIR/boltffi/"
...
export CARGO_TARGET_DIR="$ROOT_DIR/benchmarks/rust-wasm-bindgen/target"
cargo build --manifest-path "$ROOT_DIR/benchmarks/rust-wasm-bindgen/Cargo.toml" --release --target wasm32-unknown-unknown

rm -rf "$ROOT_DIR/benchmarks/rust-wasm-bindgen/dist"
mkdir -p "$ROOT_DIR/benchmarks/rust-wasm-bindgen/dist"
wasm-bindgen \
    --target nodejs \
    --out-dir "$ROOT_DIR/benchmarks/rust-wasm-bindgen/dist" \
    "$ROOT_DIR/benchmarks/rust-wasm-bindgen/target/wasm32-unknown-unknown/release/bench_wasm_bindgen.wasm"

rm -rf "$GENERATED_DIR/wasmbindgen"
mkdir -p "$GENERATED_DIR/wasmbindgen"
cp -R "$ROOT_DIR/benchmarks/rust-wasm-bindgen/dist/." "$GENERATED_DIR/wasmbindgen/"
printf '{\n  "type": "commonjs"\n}\n' > "$GENERATED_DIR/wasmbindgen/package.json"

mkdir -p "$SCRIPT_DIR/node_modules/env"
WASM_BINDGEN_WASM="$ROOT_DIR/benchmarks/rust-wasm-bindgen/dist/bench_wasm_bindgen_bg.wasm" \
...
if [[ "$PUBLISH" == true ]]; then
    "$ROOT_DIR/benchmarks/publish-benchmark-runs.sh" "$RESULTS_DIR/benchmark_run.json"
fi
