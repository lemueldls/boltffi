...
pub fn distance(a: Point, b: Point, unit: Unit) -> f64 {
  ...
  swift={`func distance(a: Point, b: Point, unit: Unit) -> Double
...
let d = distance(
  ...
  kotlin={`fun distance(a: Point, b: Point, unit: Unit): Double
...
pub fn parse_port(s: &str) -> Result<u16, ParseError> {
  ...
  swift={`func parsePort(s: String) throws -> UInt16
...
do {
  ...
  kotlin={`@Throws(ParseError::class)
