...
The `[targets.kmp]` section generates shared Kotlin declarations for `commonMain` plus JVM and
...
The shared source surface mirrors the generated Rust contract as closely as the selected ABI shape
allows. `commonMain` declares the public API, while `jvmMain` and `nativeMain` provide the
